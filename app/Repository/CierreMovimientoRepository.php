...
final class CierreMovimientoRepository
{
    private $pdo;
    private $movimientos;
    ...
    public function bloquear($movimientoId, $cuentaBancariaId, $inicioPeriodo)
    {
        if (!$this->pdo->inTransaction()) {
            throw new RuntimeException('El cierre debe ejecutarse dentro de una transaccion.');
        }
        ...
        if ($movimiento['estado_actual'] !== 'PARA_CERRAR') {
            throw new TransicionMovimientoException(
                'Solo un movimiento PARA_CERRAR puede pasar a CERRADO.'
            );
        }
    }

    public function registrar($movimientoId, $usuarioId, $clave, $efectosErp = 0, $alcance = 'SIN_EFECTO_ERP')
    {
        if (!$this->pdo->inTransaction()) {
            throw new RuntimeException('El cierre debe registrarse dentro de una transaccion.');
        }
        $consulta = $this->pdo->prepare(
        ...
        $consulta->execute([
            ...
            ':observacion' => 'API|CERRAR|' . $alcance . '|' . $clave,
        ]);
        $evento = $consulta->fetch(PDO::FETCH_ASSOC);
        if ($evento === false) {
            throw new RuntimeException('No se pudo registrar el cierre del movimiento.');
        }
