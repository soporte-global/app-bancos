...
if (!$puedePreparar || !$puedeRevertirPreparacion || !$puedeAsociarValor || !$puedeAsociarAsiento || !$puedeCrearBorrador) {
...
<div class="bandeja-mensual bancos-app" data-bandeja aria-busy="false">
    <div class="bandeja-shell">
        ...
        <p class="bandeja-introduccion"><?php echo $escrituraHabilitada && ($puedePreparar || $puedeRevertirPreparacion || $puedeAsociarValor || $puedeCrearBorrador) ? 'Consulta de extractos y preparación controlada en el sandbox.' : 'Consulta de sólo lectura de los extractos y su seguimiento.'; ?></p>
                ...
                <tr>
                    ...
                    <td data-label="Detalle"><button class="bandeja-ver-detalle" type="button" data-abrir-detalle aria-label="Ver detalle de <?php echo $escapar($movimiento['referencia']); ?>">Ver detalle</button>
                        <template data-detalle-movimiento><article class="bandeja-detalle-contenido">
                            ...
                            <?php if ($escrituraHabilitada && $puedeAsociarAsiento && $estadoCodigo === 'ABIERTO' && !$tieneAsientoAsociado): ?><section class="bandeja-detalle-seccion bandeja-accion" data-accion-asociar-asiento><h3>Asociar asiento ERP</h3><p>Reserva y vincula un asiento existente y balanceado. En modalidad exclusiva su importe debe coincidir con el movimiento; esta acción no modifica el ERP.</p><label>ID del asiento ERP<input type="number" min="1" required inputmode="numeric" data-asiento-zetti-id></label><label class="bandeja-opcion-compartida"><input type="checkbox" data-asiento-compartido><span>Asiento compartido por varios movimientos</span></label><button type="button" data-asociar-asiento data-movimiento-id="<?php echo $escapar($movimiento['id']); ?>" data-cuenta-bancaria-id="<?php echo $escapar($cuenta); ?>" data-inicio-periodo="<?php echo $escapar($periodo); ?>">Reservar y asociar asiento</button><p class="bandeja-accion-estado" role="status" aria-live="polite" data-accion-estado></p></section><?php endif; ?>
                            <?php if ($escrituraHabilitada && $puedeCrearBorrador && $estadoCodigo === 'ABIERTO' && !$tieneBorradorActivo): ?><section class="bandeja-detalle-seccion bandeja-accion bandeja-accion-borrador" data-accion-crear-borrador><h3>Crear borrador contable</h3><p>El borrador debe tener entre 2 y 200 líneas y quedar balanceado. No genera asientos en el ERP.</p><div class="bandeja-borrador-datos"><label>Nodo ERP<input type="number" min="1" required inputmode="numeric" data-borrador-nodo></label><label>Fecha contable<input type="date" required value="<?php echo $escapar($movimiento['fecha_operacion']); ?>" data-borrador-fecha></label><label>Modelo<input type="text" maxlength="80" required value="MANUAL" data-borrador-modelo></label></div><div class="bandeja-borrador-lineas" data-borrador-lineas><div class="bandeja-borrador-linea" data-borrador-linea><label>Cuenta ERP<input type="number" min="1" required inputmode="numeric" data-linea-cuenta></label><label>Debe<input type="number" min="0" step="0.00001" value="0" required data-linea-debe></label><label>Haber<input type="number" min="0" step="0.00001" value="0" required data-linea-haber></label><label>Detalle<input type="text" maxlength="200" data-linea-observacion></label><button type="button" data-quitar-linea aria-label="Quitar línea">×</button></div><div class="bandeja-borrador-linea" data-borrador-linea><label>Cuenta ERP<input type="number" min="1" required inputmode="numeric" data-linea-cuenta></label><label>Debe<input type="number" min="0" step="0.00001" value="0" required data-linea-debe></label><label>Haber<input type="number" min="0" step="0.00001" value="0" required data-linea-haber></label><label>Detalle<input type="text" maxlength="200" data-linea-observacion></label><button type="button" data-quitar-linea aria-label="Quitar línea">×</button></div></div><div class="bandeja-borrador-controles"><button type="button" data-agregar-linea>Agregar línea</button><button type="button" data-crear-borrador data-movimiento-id="<?php echo $escapar($movimiento['id']); ?>" data-cuenta-bancaria-id="<?php echo $escapar($cuenta); ?>" data-inicio-periodo="<?php echo $escapar($periodo); ?>">Crear y asociar borrador</button></div><p class="bandeja-accion-estado" role="status" aria-live="polite" data-accion-estado></p></section><?php endif; ?>
