...
$mostrarImporte = static function ($valor) {
    $texto = trim((string) $valor);
    $numero = strpos($texto, ',') !== false
        ? str_replace(',', '.', str_replace('.', '', $texto))
        : $texto;
    return is_numeric($numero) ? number_format((float) $numero, 2, ',', '.') : $texto;
};
...
$conciliacionFiltro = $filtrosResultado['conciliacion'] ?? ($_GET['conciliacion'] ?? '');
$conciliacionEtiquetas = [
    'PENDIENTE' => 'Cheque pendiente',
    'CONCILIADO' => 'Cheque conciliado',
    'NO_REQUIERE' => 'No requiere conciliación',
];
$meses = [
...
if ($conciliacionFiltro !== '' && $conciliacionFiltro !== null) {
    $filtrosActivos['conciliacion'] = 'Conciliación: ' . ($conciliacionEtiquetas[$conciliacionFiltro] ?? $conciliacionFiltro);
}
    ...
    <div class="bandeja-shell">
        ...
        <section class="bandeja-panel bandeja-selector" aria-label="Consulta de movimientos">
            <form class="bandeja-filtros" method="get" action="<?php echo $escapar(RUTA_WEB); ?>">
                ...
                <label>Conciliación
                    <select name="conciliacion"><option value="">Todas</option><?php foreach ($conciliacionEtiquetas as $codigoConciliacion => $etiquetaConciliacion): ?><option value="<?php echo $codigoConciliacion; ?>"<?php echo $conciliacionFiltro === $codigoConciliacion ? ' selected' : ''; ?>><?php echo $escapar($etiquetaConciliacion); ?></option><?php endforeach; ?></select>
                </label>
                <label>Límite
        ...
        <section class="bandeja-panel" aria-labelledby="bandeja-resultados-titulo">
            ...
            <?php foreach ($resultado->movimientos as $movimiento): ?>
                ...
                <tr>
                    <td data-label="Fecha"><?php echo $escapar($movimiento['fecha_operacion']); ?></td><td data-label="Referencia"><?php echo $escapar($movimiento['referencia']); ?></td><td data-label="Descripción"><?php echo $escapar($movimiento['descripcion']); ?></td><td class="importe" data-label="Crédito"><?php echo $escapar($mostrarImporte($movimiento['credito'])); ?></td><td class="importe" data-label="Débito"><?php echo $escapar($mostrarImporte($movimiento['debito'])); ?></td>
                    <td data-label="Estado"><span class="estado-etiqueta estado-etiqueta--<?php echo $escapar($estadoClase); ?>"><?php echo $escapar($estadoCodigo); ?></span><span class="estado-etiqueta conciliacion-etiqueta conciliacion-etiqueta--<?php echo $escapar($conciliacionClase); ?>" data-conciliacion-estado="<?php echo $escapar($conciliacionCodigo); ?>"><?php echo $escapar($conciliacionEtiqueta); ?></span></td>
                    ...
                    <td data-label="Detalle"><button class="bandeja-ver-detalle" type="button" data-abrir-detalle aria-label="Ver detalle de <?php echo $escapar($movimiento['referencia']); ?>">Ver detalle</button>
                        <template data-detalle-movimiento><article class="bandeja-detalle-contenido">
                            <header class="bandeja-detalle-resumen"><p class="bandeja-detalle-sobretitulo">Movimiento <?php echo $escapar($movimiento['referencia']); ?></p><h2><?php echo $escapar($movimiento['descripcion']); ?></h2><p><?php echo $escapar($movimiento['fecha_operacion']); ?> · Crédito <?php echo $escapar($mostrarImporte($movimiento['credito'])); ?> · Débito <?php echo $escapar($mostrarImporte($movimiento['debito'])); ?></p><div class="bandeja-detalle-estados"><span class="estado-etiqueta estado-etiqueta--<?php echo $escapar($estadoClase); ?>"><?php echo $escapar($estadoCodigo); ?></span><span class="estado-etiqueta conciliacion-etiqueta conciliacion-etiqueta--<?php echo $escapar($conciliacionClase); ?>"><?php echo $escapar($conciliacionEtiqueta); ?></span></div></header>
                            <div class="bandeja-detalle-columnas"><div class="bandeja-detalle-operaciones"><h3 class="bandeja-detalle-grupo-titulo">Operaciones</h3>
                            ...
                            <section class="bandeja-detalle-seccion bandeja-accion bandeja-accion-borrador" data-accion-crear-borrador>
                                ...
                                <div class="bandeja-borrador-lineas" data-borrador-lineas>
                                    ...
                                    <div class="bandeja-borrador-linea" data-borrador-linea>
                                        ...
                                        <label>Debe<input type="number" min="0" step="0.00001" value="0.00" required data-linea-debe></label>
                                        <label>Haber<input type="number" min="0" step="0.00001" value="0.00" required data-linea-haber></label>
                            ...
                            </div><div class="bandeja-detalle-seguimiento"><h3 class="bandeja-detalle-grupo-titulo">Seguimiento</h3>
                            <section class="bandeja-detalle-seccion" data-trazabilidad-conciliacion><h3>Trazabilidad de conciliación</h3><?php if ($conciliacionesCheque): ?><?php foreach ($conciliacionesCheque as $conciliacion): ?><article class="bandeja-conciliacion"><p><strong>Conciliación #<?php echo $escapar($conciliacion['id']); ?></strong> · <?php echo $escapar($conciliacion['estado_codigo']); ?> · <time><?php echo $escapar($conciliacion['conciliado_en']); ?></time></p><dl><div><dt>Operador</dt><dd><?php echo $escapar($conciliacion['operador'] ?: 'Usuario #' . $conciliacion['operador_hub_id']); ?></dd></div><div><dt>Cheque origen</dt><dd>#<?php echo $escapar($conciliacion['valor_origen_zetti_id']); ?></dd></div><div><dt>Operación</dt><dd>#<?php echo $escapar($conciliacion['operacion_zetti_id']); ?></dd></div><div><dt>Valor resultante</dt><dd>#<?php echo $escapar($conciliacion['valor_resultante_zetti_id']); ?></dd></div><div><dt>Asiento</dt><dd><?php echo $conciliacion['asiento_zetti_id'] !== null ? '#' . $escapar($conciliacion['asiento_zetti_id']) : 'Sin asiento'; ?></dd></div><div><dt>Modalidad</dt><dd><?php echo $conciliacion['forzada'] ? 'Forzada' : 'Estándar'; ?></dd></div></dl><?php if ($conciliacion['motivo'] || $conciliacion['evidencia_origen']): ?><small><?php echo $escapar($conciliacion['motivo'] ?: $conciliacion['evidencia_origen']); ?></small><?php endif; ?></article><?php endforeach; ?><?php else: ?><p class="sin-dato">Sin conciliaciones de cheque registradas.</p><?php endif; ?></section>
                            <section class="bandeja-detalle-seccion"><h3>Asociaciones</h3><?php if ($asociaciones): ?><ol class="bandeja-detalle-lista"><?php foreach ($asociaciones as $asociacion): ?><li><strong><?php echo $escapar($describirAsociacion($asociacion)); ?></strong><?php if ($asociacion['monto_asociado'] !== null): ?> · <?php echo $escapar($mostrarImporte($asociacion['monto_asociado'])); ?><?php endif; ?><?php if ($asociacion['compartido'] === true || $asociacion['compartido'] === 't'): ?> · compartido<?php endif; ?><?php if ($asociacion['observacion']): ?><small><?php echo $escapar($asociacion['observacion']); ?></small><?php endif; ?></li><?php endforeach; ?></ol><?php else: ?><p class="sin-dato">Sin asociaciones registradas.</p><?php endif; ?></section>
        ...
        <div class="bandeja-detalle-fondo" data-detalle-fondo hidden></div><div class="bandeja-detalle" role="dialog" aria-modal="true" aria-labelledby="bandeja-detalle-titulo" data-panel-detalle hidden><header class="bandeja-detalle-cabecera"><h2 id="bandeja-detalle-titulo">Detalle del movimiento</h2><button type="button" aria-label="Volver a la bandeja" data-cerrar-detalle>← Volver a la bandeja</button></header><div data-detalle-cuerpo></div></div>
        <?php else: ?>
