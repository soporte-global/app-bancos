...
## Ejecución y monitoreo de la migración 012
...
La tabla `global_prod.bancos_migracion_ejecucion` conserva el inicio, fin, estado y filas afectadas de cada fase. Durante una ejecución, abrir una segunda conexión y ejecutar [012_bancos_monitorear.sql](../../app/sql/migraciones/012_bancos_monitorear.sql). La fila en `EN_CURSO` identifica el bloque activo; `pg_blocking_pids` muestra bloqueadores reales y el tiempo acumulado permite distinguir una fase costosa de una espera por lock. Una cancelación revierte solamente la fase activa: las anteriores quedan confirmadas, y la siguiente ejecución es idempotente para asociaciones, reservas, borradores y mensajes.
