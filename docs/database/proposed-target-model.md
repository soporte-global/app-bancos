...
2. `bancos_movimiento_extracto`, sus asociaciones, mensajes, reservas y eventos se unen por PK interna; `id_movimiento` heredado no se reutiliza como clave. Los duplicados físicos por `id_movimiento` son idénticos y se consolidan conservando el menor `serial_seq`; la tabla de trazabilidad conserva cada fila de origen.
...
1. **Clave de migración de movimientos.** `id_movimiento` no es apta: tiene 1.055 valores duplicados y afecta asignaciones existentes. Las 2.407 filas involucradas son repeticiones exactas, por lo que el backfill conserva el menor `serial_seq` de cada ID y registra todos los orígenes. El destino genera una PK interna estable.
...
6. **Importes negativos del extracto.** Se confirmó que las 38 filas se cargaron en la columna de sentido opuesto. El backfill invierte columna y signo antes de insertarlas en el destino; el valor crudo queda trazado.
