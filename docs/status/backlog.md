...
- Repetir el catch-up dentro de la ventana definitiva de congelamiento si el legacy vuelve a recibir datos. Los lotes `catchup-2026-09-22-01` y `catchup-2026-09-22-02` agotaron el delta observado; la conciliación final quedó `APROBADO`.
- Provisionar el login secreto de depuración y concederle `app_bancos_debug_runtime`, ya creado y validado por `043`; luego reemplazar `postgres` en `config.local.php`. El DBA debe decidir si revoca globalmente `CREATE ON SCHEMA public FROM PUBLIC`, único permiso de escritura heredado que no puede negarse sólo a esta aplicación.
