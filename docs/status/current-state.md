...
Se incorporó además la base de un modo debug aún no desplegado: `bancos_debug` resuelve datos operativos y referencias ERP contra `global_temp`, mientras que el modo normal mantiene `global_prod`/`public`. La migración `014_bancos_preparar_debug_global_temp.sql` crea las estructuras vacías y secuencias independientes necesarias. Falta ejecutarla en el ambiente de depuración y recibir del DBA un usuario que no pueda escribir en los esquemas productivos.
